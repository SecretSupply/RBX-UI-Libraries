...
--# selene: allow(multiple_statements, global_usage, shadowing)

local VAPE_URL = "https://raw.githubusercontent.com/7GrandDadPGN/VapeV4ForRoblox/main/NewMainScript.lua"

local success, res = pcall(game.HttpGet, VAPE_URL)
...
local friendrecolor = Friends.CreateToggle({
	...
	["Function"] = function() FriendsTextList.FriendColorRefresh:Fire() end,
	["Default"] = true
})
...
local function UpdateHud()
	if GuiLibrary["MainGui"].ScaledGui.Visible then
		...
		local textlists = {}
		for _, v2 in pairs(tableofmodules) do
			if first then
...
local TextGuiUseCategoryColor = TextGui.CreateToggle({
	...
	["Function"] = function() GuiLibrary["UpdateUI"]() end
})
TextGui.CreateToggle({
	["Name"] = "Watermark", 
...
GUI.CreateColorSlider("GUI Theme", function(val) GuiLibrary["Settings"]["GUIObject"]["Color"] = val GuiLibrary["UpdateUI"]() end)
